// New schemas file: schemas/user.schema.js
...
const profileSchema = Joi.object({
  ...
  emergencyContactPhone: Joi.string().pattern(/^[0-9+\-\s()]{10,20}$/).messages({
  ...
  // Travel Preferences
  dietaryRestrictions: Joi.array().items(Joi.string().max(50)).max(20),
  mobilityNeeds: Joi.array().items(Joi.string().max(50)).max(10),
  accessibilityNeeds: Joi.array().items(Joi.string().max(50)).max(10),
  preferredTravelStyle: Joi.string().valid(...travelStyleEnum).messages({
    'any.only': `Travel style must be one of: ${travelStyleEnum.join(', ')}`
  }),
  travelCompanionPref: Joi.string().valid(...travelCompanionEnum).messages({
  ...
  // Social Media Links
  twitterHandle: Joi.string().max(50).pattern(/^@?(\w){1,15}$/).messages({
    'string.pattern.base': 'Please provide a valid Twitter handle'
  }),
  instagramHandle: Joi.string().max(50).pattern(/^@?([\w.]){1,30}$/).messages({
    'string.pattern.base': 'Please provide a valid Instagram handle'
  }).optional(),
  facebookProfile: Joi.string().uri().max(200).messages({
    'string.uri': 'Please provide a valid Facebook profile URL'
  }).optional(),
  linkedInProfile: Joi.string().uri().max(200).messages({
    'string.uri': 'Please provide a valid LinkedIn profile URL'
  }).optional(),

  // Statistics
  totalTripsPlanned: Joi.number().integer().min(0).default(0),
  totalTripsCompleted: Joi.number().integer().min(0).default(0),
  countriesVisited: Joi.array().items(Joi.string().max(100)).max(300),
  favoriteDestinations: Joi.array().items(Joi.string().max(100)).max(50)
});
...
// ==================== Cache Clear Query Schema ====================
const clearCacheQuerySchema = Joi.object({
  userId: Joi.string().uuid().optional(),
  pattern: Joi.string().optional()
}).or('userId', 'pattern'); // At least one must be provided
